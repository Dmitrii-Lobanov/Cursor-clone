...
const paramsSchema = z.object({
    urls: z.array(z.url()).min(1, 'Invalid URL format')
...
export const createScrapeUrlsTool = () => {
    return createTool({
        name: 'scrapeUrls',
        description: `Scrape content from URLs to get documentation or reference material. 
        Use this when the user provides URLs or references external documantation. 
        ...
        handler: async (params, { step: toolStep }) => {
            ...
            try {
                return await toolStep?.run('scrape-urls', async () => {
                    ...
                    if (results.length === 0) {
                        return `No content could be scraped form the provided URLs`;
                    }
