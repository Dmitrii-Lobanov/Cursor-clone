...
export const createFolder = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        // Check if the folder with the same name already exists in the parent folder
        const files = await ctx.db
            .query("files")
            .withIndex('by_project_parent', 
                q => q.eq('projectId', args.projectId)
...
export const renameFile = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        // Check if the file with the same new name already exists in the parent folder
...
export const deleteFile = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        // Recursively delete file or folder and all descendents
        const deleteRecursive = async (fileId: Id<'files'>) => {
            ...
            if (item.type === 'folder') {
                const children = await ctx.db.query('files')
                    .withIndex('by_project_parent', (q) => q.eq('projectId', item.projectId)
                    .eq('parentId', fileId))
                .collect();
